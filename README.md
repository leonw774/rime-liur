...
> 就我的習慣重新設計了配置
> - 橫向侯選欄
> - 預設schema list設成只有liur和注音
> - 刪除拼音模式，因為我用不到
> - 快捷鍵
>   - 刪除 `ctrl + /` 、 `ctrl + '` 、 `ctrl + .` 三個切換鍵，因為會和很多軟體的快捷鍵衝突
>   - 查碼現在透過`;'`和`';`前綴實現，簡繁轉換和擴充字集切換仍然可以在`` ctrl + ` ``的選單中使用
>   - 新增我習慣的選單瀏覽快捷鍵：`shift + <` 為 `Page_Up` 、 `shift + >` 為 `Page_Down`
> - 注音到蝦米的反查不再基於opencc: 現在只能一次查一字，但格式更好看
> - 修改了注音、注音到蝦米反查、蝦米到注音反查、造詞模式的前綴，分別是`;;`、`;'`、`';`、`;;;`，這樣好記多了，而且造詞模式使用頻率低，長度應較長才比較合理，也不會不小心開啟造詞模式
> - 有關中英混輸的改動
>   - 預設輸入英文
>   - 刪除`weasel.custom.yaml`裡的`app_options`
>   - 左 shift 切換中英輸入模式，並吐出編輯區裡的編碼
